  ...
  (cases continuation k
    ...
    (let1-cont (vars body env saved-cont)
               (trampoline
                (lambda ()
                  (create-let-lenv
                    vars val env (let2-cont body saved-cont)))))
    ...
    (rator-cont (rands env saved-cont)
                (trampoline (lambda () (eval-rands/k rands env (rands-cont val saved-cont)))))
    ...
    (eval-rands-cont1 (rands env saved-cont)
                      (trampoline (lambda () (eval-rands/k rands env (eval-rands-cont2 val saved-cont)))))
...
(define (eval-rands/k rands env k)
  (if (null? rands)
      ...
      (trampoline (lambda () (value-of/k (car rands) env (eval-rands-cont1 (cdr rands) env k))))))
...
(define (create-let-lenv vars vals env k)
  (if (empty? vars)
      ...
      (trampoline (lambda () (create-let-lenv (cdr vars)
                                              (cdr vals)
...
(define (value-of-program pgm)
  (cases program pgm
    (a-program (exp1)
               (trampoline (lambda () (value-of/k exp1 (empty-env) (end-cont)))))))
