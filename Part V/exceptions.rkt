...
(define-datatype continuation cont?
  ...
  (try-cont
   (var symbol?)
   (handler-exp expression?) ;; evaluate this if evaluation doesn’t end normally
   (env environment?)
   (cont cont?))

  (raise1-cont (saved-cont cont?)) ;; save current cont to find handler (saved in a try-cont)
  )
...
(define (report-uncaught-exception)
  (begin (display "UNCAUGHT EXCEPTION!") (newline)))
...
(check-equal? (eval "let f = proc (n)
                              if zero?(n)
                              then raise -1
                              else n
                         in try (f 0) catch (i) -(i, 1)")
              (num-val -2))
